...
let meshingFile = fs.readFileSync(
  path.join(__dirname, "meshing.json"),
  "utf-8"
);
let meshingText = meshingFile.toString().trim();
let meshing = meshingText === "" ? [] : JSON.parse(meshingText);
for (let i = 0; i < meshing.length; i++) {
  nodes.push(meshing[i]);
  console.log(nodes);
}
...
app.all("/proxy/:node/*", memoryUpload.single("file"), async (req, res) => {
  ...
  if (!targetNode) return res.status(404).send("Node not found");

  const restPath = req.params[0];
  const targetUrl = `http://${targetNode.ip}/${restPath}`;
  const headers = { "x-password": req.headers["x-password"] || "" };
  ...
  try {
    ...
    try {
      ...
      console.warn(`[proxy] Cannot reach ${targetUrl}: ${err.code || err.message}`);
      if (!res.headersSent) {
        res.status(502).send("Node unreachable");
    ...
    // Custom logic
    if (response.status === 200 && restPath.startsWith("setInstance")) {
      globalCurrentInst["name"] = decodeURIComponent(req.params.name);
      globalCurrentInst["node"] = nodeName;
      console.log(`${globalCurrentInst.name} ${globalCurrentInst.node}`);
    }

    // Handle streaming safely
    response.data.on("error", (streamErr) => {
      console.warn(`[proxy] Stream error from ${targetUrl}: ${streamErr.code || streamErr.message}`);
      if (!responseHandled) {
        if (!res.headersSent) res.status(500).send("Stream error");
        else res.end(); // ensure connection closes
        responseHandled = true;
      }
    });

    // Close listener if client aborts
    res.on("close", () => {
    ...
    console.error("[proxy] Fatal proxy error:", err?.code || err?.message || String(err));
    ...
    // Ensure connection doesn’t hang forever
    if (!responseHandled && !res.headersSent) {
      res.status(500).send("Proxy failed unexpectedly");
    } else if (!responseHandled && res.writableEnded === false) {
      res.end();
    }
  }
});
...
app.get("/terminal", (req, res) => {
  ...
  if (cmd === "start") {
    ...
    allProcesses[curInstId()].stderr.on("data", (data) =>
      broadcastConsole(data.toString())
    );
...
wss.on("connection", (ws) => {
  ...
  ws.send(
    JSON.stringify({
      type: "info",
      data: "Connected to console. Please authenticate.",
    })
  );
    ...
    if (data.type === "auth") {
      if (stringtokens.includes("," + data.password + ",")) {
        ...
        ws.send(
          JSON.stringify({
            type: "info",
            data: "Authorized. Console access granted.",
          })
        );
      } else {
...
app.post("/linkNode", (req, res) => {
  ...
  for (let i = 0; i < nodes.length; i++) {
    if (nodes[i].name == name1) {
      res
        .status(400)
        .send("Error: Node with name " + name1 + " already exists.");
      return;
    }
  }
  nodes.push({
...
app.post("/unlinkNode", (req, res) => {
  ...
  for (let i = 0; i < nodes.length; i++) {
    if (nodes[i].name == name1) {
      if (nodes[i].isMain) {
        res.status(400).send("Error: You cannot unlink the pannel host node.");
        return;
      } else {
        nodes.splice(i, 1);
        break;
...
async function pingNodes() {
  try {
    const promises = nodes
      ...
      .map(async n => {
        try {
          const res = await axios.get(`http://${n.ip}/ping`, {
            httpAgent: agent,
            timeout: 5000,
            validateStatus: () => true, // never throw on non-2xx status
          });
          return { node: n, response: res };
        } catch (err) {
          // Catch all network-level issues
          return { node: n, error: err };
        }
      });

    const results = await Promise.all(promises);

    for (const { node, response, error } of results) {
      try {
        if (error) {
          console.warn(`[pingNodes] ${node.name} (${node.ip}) ${error.message}`);
          node.isOnline = false;
        } else if (response?.status === 200) {
          node.isOnline = true;
          const d = response.data;
          node.memory = d.freeMemory;
          node.totalMemory = d.totalMemory;
          node.diskSpace = d.freeDiskSpace;
          node.totalDiskSpace = d.totalDiskSpace;
          node.processes = d.processes;
        } else {
          console.warn(`[pingNodes] ${node.name} (${node.ip}) ${response?.statusText || "Bad response"}`);
          node.isOnline = false;
        }
      } catch (err) {
        console.error(`[pingNodes] Internal node loop error for ${node.name}:`, err.message);
      }
    }
  } catch (err) {
    console.error("[pingNodes] Fatal error:", err.message);
  }

  // --- Main node info ---
  const mainNode = nodes[0];
  try {
    const drivePath = os.platform() === "win32"
      ? path.parse(__dirname).root.replace(/\\$/, "")
      : "/";
    const info = disk.checkSync(drivePath);
    const totalMemory = os.totalmem();
    const freeMemory = os.freemem();

    const pcount = allProcesses.filter(p => p && p !== "null").length;

    mainNode.totalMemory = totalMemory;
    mainNode.memory = freeMemory;
    mainNode.totalDiskSpace = info.total || 0;
    mainNode.diskSpace = info.available || 0;
    mainNode.processes = pcount;
  } catch (err) {
    console.log("[pingNodes] Failed to get main node info:", err.message);
  }
}
...
app.get("/currentInstInfo", async (req, res) => {
  try {
    ...
    if (globalCurrentInst["node"] != nodes[0].name) {
      const targetNode = nodes.find(n => n.name === globalCurrentInst["node"]);
      if (!targetNode) {
        return res.status(404).send("Target node not found");
      }

      const targetUrl = `http://${targetNode.ip}/currentInstInfo?name=${encodeURIComponent(globalCurrentInst["name"])}`;
      ...
      try {
        ...
        // Send remote response (even if non-200)
        res.status(response.status).send(response.data);
      } catch (err) {
        console.warn(`[currentInstInfo] Failed to reach ${targetUrl}: ${err.code || err.message}`);
        res.status(502).send("Node unreachable");
        globalCurrentInst["name"] = getLatestInstance();
        globalCurrentInst["node"] = nodes[0].name;
...
app.get("/instanceList", (req, res) => {
  ...
  for (var i = 0; i < instanceList.length; i++) {
    ...
    instanceList[i]["node"] = nodes[0].name;
    ...
    try {
      ...
      try {
        ...
        if (allowedidsstring.includes("," + userData.id + ",")) {
          ...
          globalstrTokens = stringtokens;
          //push to all nodes besides the one at index 0 via http
...
setInterval(() => {
  ...
  allowedidsstring = fs.readFileSync(
    path.join(__dirname, "allowedids.txt"),
    "utf-8"
  );
}, 1000);

async function updateTokens(stringtokens)
{
  for (let i = 1; i < nodes.length; i++) {
            try {
              let res = await fetch("http://" + nodes[i].ip + "/token", {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
                  secret: nodes[i].secret,
                },
                body: JSON.stringify({
                  token: stringtokens,
                  name: nodes[i].name,
